...
//        writeToLog("get product:".json_encode($selectedRow));
    ...
    if($mainProductQuantity - $quantity < 0)
    {
        ...
        $ret["quantity"] = getProductQuantity($sku);
        $ret["sku"] = $sku;
        $ret["message"] = "Sku: $sku\r\nจำนวนสินค้านี้ในระบบเหลือ $mainProductQuantity";
        mysqli_close($con);
...
//    $sql = "update mainProduct set quantity = '$mainProductQuantity' where sku = '$sku'";
    $sql = "update mainProduct set quantity = '$mainProductQuantity' where sku in (select sku from stocksharing where stocksharinggroupID in (select stocksharinggroupID from stocksharing where sku = '$sku')) or sku = '$sku'";
    ...
    for($i=0; $i<sizeof($selectedRow); $i++)
    {
        ...
        $ret = updateStockQuantityLazadaInApp($shareSku,$mainProductQuantity);
        if(!$ret)
        {
            //check the reason if it's because of sku_not_found
            $lazadaProduct = getLazadaProduct($shareSku);
            if(!$lazadaProduct)
            {
                //delete
        ...
        $ret = updateStockQuantityShopeeInApp($shareSku,$mainProductQuantity);
        if(!$ret)
        {
            //check the reason if it's because of sku_not_found
            if(!$variations)
        ...
        $ret = updateStockQuantityJdInApp($shareSku,$mainProductQuantity);
        if(!$ret)
        {
            //check the reason if it's because of sku_not_found
    ...
    echo json_encode(array("success"=>$success, "message"=>$message, "sku"=>$sku, "quantity"=>$asIsQuantity, "quantityOut"=>$quantityOut, "mainImage"=>$mainImage, "stockSharingList"=>$stockSharingList));
    exit();
