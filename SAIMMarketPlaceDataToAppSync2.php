...
//    writeToLog("set auto commit to off");
    ...
    {
        $sql = "select SellerSku as Sku from lazadaproducttemp where status = 'active' and SellerSku not in (select sku from mainproduct)";
...
//        }
        ...
        {            
            ...
            //insert mapsku
            $sql = "INSERT INTO `mapsku`( `Sku`, `LazadaSku`, `ShopeeSku`, `JdSku`, `WebSku`, `ModifiedUser`) select '$escapeSku', '$escapeSku', '$escapeSku', '$escapeSku', '$escapeSku','bot'";
            ...
            {
                //update main
                $sql = "update mainProduct set Name = '$name',PrimaryCategory='$primaryCategory', brand = '$brand', sku = '$escapeSku', specialPrice = '$specialPrice', price = '$price', mainImage = '$mainImage', image2 = '$image2', image3 = '$image3', image4 = '$image4', image5 = '$image5', image6 = '$image6', image7 = '$image7', image8 = '$image8', quantity = '$quantity', modifiedUser = '$modifiedUser' where sku = '$escapeSku'";
                $ret = doQueryTask($con,$sql,$modifiedUser);
                if($ret != "")
                {
                    $updateFail[] = $sku;
            ...
            if(!$ret)
            {
                $sql = "insert into lazadaProduct (Sku,ModifiedUser) values( '$escapeSku','$modifiedUser')";
                $ret = doQueryTask($con,$sql,$modifiedUser);
                if($ret != "")
                {
        ...
        if($hasJdSku)
        {
            ...
            if(!$ret)
            {
                $sql = "insert into jdProduct (`Sku`, `ProductId`, `SkuId`, `ModifiedUser`) values('$escapeSku','$productId','$skuId','$modifiedUser')";
                $ret = doQueryTask($con,$sql,$modifiedUser);
                if($ret != "")
                {
                    $insertFailJd[] = $sku;
...
//    sleep(40);
    ...
    writeToLog("insert map fail:" . json_encode($insertFailMap));    
