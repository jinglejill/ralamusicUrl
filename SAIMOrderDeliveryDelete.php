<?php
    ...
    $orderDeliveryID = json_decode($json_str,true)["orderDeliveryID"];
    ...
    //OrderDelivery
    $sql = "select * from OrderDelivery where orderDeliveryID = '$orderDeliveryID'";
    ...
    //delete
    $sql = "delete from OrderDeliveryItem where orderDeliveryID = '$orderDeliveryID'";
    $ret = doQueryTask($con,$sql,$modifiedUser);
    if($ret != "")
    {
        $ret["message"] = "ลบ order ไม่สำเร็จ";
        mysqli_close($con);
    ...
    $sql = "delete from OrderDelivery where orderDeliveryID = '$orderDeliveryID'";
    $ret = doQueryTask($con,$sql,$modifiedUser);
    if($ret != "")
    {
        $ret["message"] = "ลบ order ไม่สำเร็จ";
